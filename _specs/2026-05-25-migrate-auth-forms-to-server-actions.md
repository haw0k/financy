...
- Should we use `revalidatePath` or `router.refresh()` after successful auth to ensure middleware re-evaluates session?
- Should the `useRole` hook remain client-side or also move to server-side data fetching?
