...
- Data Server Actions (categories, transactions, dashboard) must reject unapproved users and admin users at the action boundary via `requireApprovedUser`
- Admin actions must validate the target user (valid UUID, non-admin, pending status) before approve/reject
- Admin sign-up must use only the configured redirect URL and fail if it is missing
- Admin action errors from the service-role client must be mapped through the centralized error mapper
...
- Data Server Actions reject pending users and admin users with a clear message
- Admin actions validate target UUID, role, and status before mutating
- Admin sign-up fails early when redirect URL is not configured
- Admin action errors from the service-role client are sanitized
